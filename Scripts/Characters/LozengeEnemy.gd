...
const DamageParticles = preload("res://Scenes/Particles/OrangeDamage.tscn")
const TriangleEnemy = preload("res://Scenes/Characters/TriangleEnemy.tscn")
const SquareEnemy = preload("res://Scenes/Characters/SquareEnemy.tscn")
...
func _on_LozengeEnemy_body_entered(body):
	if "SquareProjectile" in body.get_name() or "TriangleProjectile" in body.get_name():
...
func spawnRandomMobs() -> void:
	var spawnPoisiton1 = $SpawnPosition1.global_position
	var spawnPosition2 = $SpawnPosition2.global_position
	
	var SquareEnemyInstance = SquareEnemy.instance()
	get_tree().current_scene.add_child(SquareEnemyInstance)
	
	var TriangleEnemyInstance = TriangleEnemy.instance()
	get_tree().current_scene.add_child(TriangleEnemyInstance)
	
	SquareEnemyInstance.position = spawnPoisiton1
	TriangleEnemyInstance.position = spawnPosition2
